...
function SchedaTecnica({
  ...
  const handleNuovaScheda = async (tipo) => {
    try {
      ...
      const giaEsiste =
        tipo.categoria === 'tecnica' ? esistenti.find((s) => s.tipo_id === tipo.id) : null;
            ...
            <div className="flex-column-center ">
              ...
              {!loading && (
                <div className="flex-column-center" style={{ marginTop: 20 }}>
                  ...
                  {schede.filter((s) => (tipoIdToCategoria?.[s.tipo_id] ?? s.categoria) === 'multi')
                    .length === 0 ? (
              ...
              {!loading && (
                <div className="flex-column-center" style={{ marginTop: 20 }}>
                  ...
                  {schede.filter(
                    (s) => (tipoIdToCategoria?.[s.tipo_id] ?? s.categoria) === 'tecnica'
                  ).length === 0 ? (
                    <div style={{ fontSize: 14, opacity: 0.7 }}>
                      Nessuna scheda multi creata per questa commessa.
                    </div>
