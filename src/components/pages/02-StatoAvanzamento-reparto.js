...
import {
  repartoConfig,
  getBoardCustomFieldId,
  getCablQeValueFromCard,
  repartoIndicators,
} from '../config/repartoConfig';
...
function StatoAvanzamentoReparti() {
  ...
  const toggleCommessaExpanded = (commessaId) => {
  ...
  const getAxiosErrorMessage = (error) => {
    ...
    if (status === 409)
      return 'Spostamento rifiutato: la commessa è stata aggiornata da un altro utente.';
  ...
  const deleteNote = async (activityId) => {
    const first = window.confirm(`ATTENZIONE: vuoi ELIMINARE DEFINITIVAMENTE la nota?`);
    if (!first) return;
  ...
  */
  ...
  const refetchCommesse = async () => {
    ...
    const parsed = response.data.map((c) => ({
      ...c,
      stati_avanzamento:
        typeof c.stati_avanzamento === 'string'
          ? JSON.parse(c.stati_avanzamento)
          : c.stati_avanzamento,
    }));
    setCommesse(parsed);
    return parsed;
    ...
    return (
      ...
      >
        ...
        <div className="commessa-header">
          <div className="commessa-header-top">
            <div className="commessa-header-main">
              <strong>{commessa.numero_commessa}</strong>
              <div>{commessa.cliente || '-'}</div>
            </div>
          </div>

          <div className="commessa-badges row center" style={{ paddingTop: '0px' }}>
          ...
          {viewMode === 'compact' && (
            <div className="commessa-badges commessa-badges--compact row center">
              ...
              {confrontoConTrello && trelloCard && isListDifferent && (
                <span title="Lista Trello diversa">📌</span>
              )}

              {confrontoConTrello && trelloCard && isDateDifferent && (
                <span title="Data diversa">📅</span>
              )}
            </div>
          )}
        </div>
        <div className=" row center" style={{ paddingTop: '10px' }}>
          {viewMode === 'compact' && hasExpandableDetails && (
            <button
              type="button"
              className="commessa-expand-btn"
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                onToggleExpanded();
        ...
        <div
          className={`commessa-details ${
            viewMode === 'full' || isExpanded
              ? 'commessa-details--open'
              : 'commessa-details--closed'
          }`}
        >
          {allarmiNote && warningActivities.length > 0 && (
            <WarningDetails
        ...
        {draggingCommessaId ? (
          ...
          commesse.map((commessa) => (
            <DraggableCommessa
              ...
              resources={resources}
              viewMode={viewMode}
              isExpanded={!!expandedCommesse[commessa.commessa_id]}
              onToggleExpanded={() => toggleCommessaExpanded(commessa.commessa_id)}
            />
          ))
        )}
      </div>
  ...
  return (
    <div className="page-wrapper">
      ...
      {ctxMenu.visible && (
        ...
        >
          <div style={{ padding: '8px 10px', fontSize: 12, opacity: 0.8 }}>Sposta commessa</div>
