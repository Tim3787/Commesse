...
import { fetchSchedeTecniche, deleteSchedaTecnica } from '../services/API/schedeTecniche-api';
...
function SchedeTecnicheTable() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      const clickedOutsideFiltro =
        !event.target.closest('.suggestions-list') && !event.target.closest('.w-200');
  ...
  const handleDelete = async (schedaId) => {
    if (
      window.confirm(
        'Sei sicuro di voler eliminare questa scheda? Elminando la scheda, anche tute le note verranno perse'
      )
    ) {
        ...
        <div className="Reparto-table-container mh-80 ">
            ...
            <tbody>
              {applyFilters().map((scheda) => (
                <tr key={scheda.id}>
                  ...
                  <td>{scheda.tipo}</td>
                  <td>
                    {scheda.creato_da_nome} il {formatDateTime(scheda.data_creazione)}
                  </td>
                  <td>{formatDateTime(scheda.data_modifica)}</td>
                  <td>
                    <button
                      className="btn w-100 btn--warning btn--pill"
                      onClick={() => handleEditScheda(scheda)}
                    >
                      Modifica
                    </button>
                    <button
          ...
          {showPopup && (
            <SchedaTecnica
              ...
              numero_commessa={
                selectedCommessa?.numero_commessa || schedaInModifica?.numero_commessa
              }
