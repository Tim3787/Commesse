...
import {
  ...
  fetchDashboardActivityById,
  fetchDeptActivityById,
} from '../services/API/dashboard-api';
import { fetchUserName } from '../services/API/utenti-api';
import {
  fetchMyOpenNotes,
  fetchMyOpenActivities,
  fetchRepartoDashboard,
    ...
    try {
      ...
      setLinkedNotesByActivity((prev) => ({
        ...
        [a.id]: {
          loading: false,
          loaded: true,
          error: 'Errore caricamento note collegate.',
          notes: [],
        },
      }));
    } finally {
    ...
    <div>
      <div className="container">
        ...
        <div className="user-dash">
            ...
            {deptData && (
              <div className="flex-column-center">
                ...
                {deptData.openActivities.map((a) => (
                  ...
                  >
                    <span>
                      <strong>Commessa:</strong> {a.numero_commessa}
                    </span>
                    <span>—</span>
                    <span>
                      <strong>Attività:</strong> {a.nome_attivita}
                    </span>
                    <span>—</span>
                    <span>
                      <strong>Iniziata il:</strong>{' '}
                      {new Date(a.data_inizio).toLocaleDateString('it-IT')}
                    </span>
                    <span>—</span>
                    <span>
                      <strong>Da:</strong> {a.risorsa_nome || '—'}
                    </span>
                  </li>
                ))}
                ...
                {deptData.openNotes.map((a) => (
                  ...
                  >
                    ...
                    <span>
                      <strong>Creata da:</strong> {a.risorsa_nome || '—'}
                    </span>
                  </li>
                ))}
              </div>
          ...
          <div className="flex-column-center">
            <button
              className="btn btn--pill w-400 btn--blue"
              onClick={() => navigate('/00-Dashboard-user-calendar')}
            >
              Vai al calendario
            </button>
                                    ...
                                    {pack.notes.map((n) => (
                                      ...
                                      >
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                                          ...
                                          <span>—</span>
                                          <span>
                                            {new Date(n.data_inizio).toLocaleDateString('it-IT')}
                                          </span>
                                          <span>—</span>
